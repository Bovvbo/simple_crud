...
def student_add(name=None):
    return render_template("student_add.html", name=name)
...
def student_add_submit():
    ...
    # use %s not ? if you use mysql
    insert_stmt = "insert into students(name, photo_path) " "values(?, ?)"
    data = (request.form["name"], photo_path)
...
def submit_student_upd():
    ...
    if photo and photo.filename:
        ...
        upd_stmt = "update students " "set name=?, photo_path=? " "where id=?"
        data = (request.form["name"], photo_path, request.form["id"])
    else:
        upd_stmt = "update students " "SET name=? " "where id=?"
